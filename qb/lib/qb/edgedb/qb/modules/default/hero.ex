defmodule(QB.EdgeDB.QB.Modules.Default.Hero) do
  [
    (
      @properties [QB.EdgeDB.QB.Modules.Default.Person]
                  |> Enum.map(fn base -> base.__properties__() end)
                  |> Enum.concat([:number_of_movies, :secret_identity])
                  |> List.flatten()
      ...
      @backlinks [QB.EdgeDB.QB.Modules.Default.Person]
                 ...
                 |> Enum.concat([:"<nemesis[is Villain]"])
                 |> List.flatten()
      ...
      (
        object_name = :hero
        type_id = "c16608fe-91d9-11ec-bb2a-ab2c337ace3c"
