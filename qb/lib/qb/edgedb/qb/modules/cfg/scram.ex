defmodule(QB.EdgeDB.QB.Modules.Cfg.SCRAM) do
  [
    (
      @properties [QB.EdgeDB.QB.Modules.Cfg.AuthMethod]
                  |> Enum.map(fn base -> base.__properties__() end)
                  |> Enum.concat([])
                  |> List.flatten()
      @link_properties [QB.EdgeDB.QB.Modules.Cfg.AuthMethod]
                       |> Enum.map(fn base -> base.__link_properties__() end)
                       |> Enum.concat([])
                       |> List.flatten()
      @links [QB.EdgeDB.QB.Modules.Cfg.AuthMethod]
             |> Enum.map(fn base -> base.__links__() end)
             |> Enum.concat([])
             |> List.flatten()
      @backlinks [QB.EdgeDB.QB.Modules.Cfg.AuthMethod]
                 |> Enum.map(fn base -> base.__backlinks__() end)
                 |> Enum.concat([])
                 |> List.flatten()
      @backlink_stubs [QB.EdgeDB.QB.Modules.Cfg.AuthMethod]
      ...
      (
        object_name = :scram
        type_id = "72446121-8a10-11ec-a0ee-0f0637eee6d9"
