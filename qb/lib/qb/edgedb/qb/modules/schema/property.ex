defmodule(QB.EdgeDB.QB.Modules.Schema.Property) do
  [
    (
      @properties [QB.EdgeDB.QB.Modules.Schema.Pointer]
                  |> Enum.map(fn base -> base.__properties__() end)
                  |> Enum.concat([])
                  |> List.flatten()
      @link_properties [QB.EdgeDB.QB.Modules.Schema.Pointer]
                       |> Enum.map(fn base -> base.__link_properties__() end)
                       |> Enum.concat([])
                       |> List.flatten()
      @links [QB.EdgeDB.QB.Modules.Schema.Pointer]
             |> Enum.map(fn base -> base.__links__() end)
             |> Enum.concat([])
             |> List.flatten()
      @backlinks [QB.EdgeDB.QB.Modules.Schema.Pointer]
                 |> Enum.map(fn base -> base.__backlinks__() end)
                 |> Enum.concat([
                   :"<properties[is schema::Link]",
                   :"<properties[is schema::ObjectType]"
                 ])
                 |> List.flatten()
      @backlink_stubs [QB.EdgeDB.QB.Modules.Schema.Pointer]
      ...
      (
        object_name = :property
        type_id = "6ffb3e67-8a10-11ec-9d89-5f1de95272c4"
